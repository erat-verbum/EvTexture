...
from src.cli import download as download_module, infer, utils
...
def main(
    ...
    if utils.is_video_file(input):
        ...
        if verbose:
            ...
            if deinterlace:
                click.echo("  - Deinterlaced: Yes (ffmpeg)")
        ...
        raise click.ClickException(
            "Input must be a video file or directory containing images"
        )
    ...
    if output_path.suffix.lower() in [".mp4", ".avi", ".mov", ".mkv"]:
        temp_output_dir = tempfile.mkdtemp(prefix="evtexture_")
    ...
    elif temp_output_dir is None:
        if verbose:
            click.echo(f"  - Frames already saved incrementally to: {output_path}")
